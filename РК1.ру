...
def RequestThree(operators, languages, operators_languages):
    # Используем list comprehension + сортировку
    result = [
...
print(" Операторы, начинающиеся с 'b', и их языки программирования:")
for content, lang in RequestOne(operators, languages):
    print(f"Оператор: {content}, Язык: {lang}")

print("\n Список длин операторов (по возрастанию):")
print(RequestTwo(operators))

print("\n Все операторы и их языки (многие-ко-многим):")
for op_id, content, lang in RequestThree(operators, languages, operators_languages):
    print(f"ID: {op_id}, Оператор: {content}, Язык: {lang}")
